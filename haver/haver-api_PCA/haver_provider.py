...
def _metadata_request(ticker_list, log_error=True):
    try:
        ...
        # Haver.metadata raises on invalid inputs (e.g. math-function tickers
        # like INTERP(...)). Swallow here so a single bad ticker can't abort the
        # whole batch — the caller's per-ticker fallback recovers the good ones.
        if log_error:
            log_event(logger, "warning", "Metadata request raised", error=str(exc), ticker_count=len(ticker_list))
        return pd.DataFrame()
