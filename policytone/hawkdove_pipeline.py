# -*- coding: utf-8 -*-
"""
중앙은행 매파/완화(Hawk-Dove) 스코어링 파이프라인 — 미국·일본
=========================================================================
백테스트용: '글(연설) 단위' 채점.

흐름:  scrape → filter → LLM 3축 채점 → stance 공식 계산 → 일별 집계
       → 지수화(z-score±100) → 백테스트

[종합 stance = 공식 계산]
   stance = 0.5*policy_path + 0.3*inflation + 0.2*growth_labor
   · LLM은 3축(inflation/growth_labor/policy_path)만 채점
   · stance 는 파이썬에서 계산 → 100% 재현 가능 (백테스트에 유리)

[기타 합의]
   · LLM 판단 + 사전은 참고 닻 / few-shot 앵커 / confidence 기준 명시
   · 발화 시점 기준(사후 보정 금지) / 글 단위
   · 집계: 단순평균(화자 수 가중 없음) / 평활: 이동평균(MA)
   · 지수화: 중앙은행별 롤링 z-score → ±100

실행:
  pip install -r requirements.txt
  (.env 에 ANTHROPIC_API_KEY=sk-ant-...  또는  export ANTHROPIC_API_KEY=...)
  python hawkdove_pipeline.py
"""
...
if __name__ == "__main__":
    ...
    for bank in BANKS_TO_RUN:
        for year in range(START_YEAR, END_YEAR + 1):
            ...
            print(f"   통화정책 연설 {len(speeches)}건 채점 시작")
            for s in speeches:
                if len((s.get("body") or "")) < 500:
                    continue
                try:
                    sc = score_speech(s["body"])
                    axes = sc["axes"]
                    stance = compute_stance(axes)        # ← 공식으로 계산
                    all_scored.append({
                        "date": s["date"], "bank": bank,
                        "speaker": s.get("speaker", ""), "title": s.get("title", ""),
                        "url": s.get("url", ""),
                    ...
                    print("   채점 실패:", e)
            # 연도(은행)마다 중간 저장 → 끊겨도 누적분 보존
            save_results(all_scored)
            print(f"   [중간저장] 누적 {len(all_scored)}건 → speech_scores.csv")
            time.sleep(1)
    ...
    if os.path.exists(RATES_CSV):
        ...
        for bank, yld in [("FED", "us2y"), ("BOJ", "jp2y")]:
